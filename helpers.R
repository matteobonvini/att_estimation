...
# get_eff_bound <- function(alpha, gamma, beta, p, att) {
#   
#   y.given.x.z0 <- function(y, x) {
#     
#     pi.xtrunc <- ifelse(pi.x(alpha, x, TRUE) > 0.999, 0.999, pi.x(alpha, x, TRUE))
#     
#     den <- ( dnorm(y, mean = mean.y(x, 0, beta)) )*(1 - pi.xtrunc)*dnorm(x)
#     
#     return( ( pi.xtrunc/(1-pi.xtrunc)*(y - mean.y(x,0,beta)) )^2*den )
#     
#   }
#   
#   y.given.x.z1 <- function(y, x) {
#     
#     den <- ( dnorm(y, mean = mean.y(x, 1, beta))*lambda1.x(gamma, x, TRUE) + 
#                dnorm(y, mean = mean.y(x, 0, beta))*(1 - lambda1.x(gamma, x, TRUE)) )*pi.x(alpha, x, TRUE)*dnorm(x)
#     
#     return( ( (y-mu1.x(gamma, beta, x, TRUE)) )^2*den )
#     
#   }
#   
#   inner.fn1 <- Vectorize( function(x) { integrate(function(y) { y.given.x.z1(y,x) }, 
#                                                   -100, 100)$value }
#   )
#   
#   inner.fn0 <- Vectorize( function(x) { integrate(function(y) { y.given.x.z0(y,x) }, 
#                                                   -Inf, Inf)$value }
#   )
#   
#   # Might be violations of positivity for larger values of x and y
#   var.y1 <- integrate(inner.fn1, -30, 30)$value
#   
#   var.y0 <- integrate(inner.fn0, -500, 500)$value
#   
#   
#   # Compute the other quantities of the efficiency bound. 
#   int.fn2 <- function(x) {
#     
#     dnorm(x)/p^2*((pi.x(alpha,x, TRUE)*(mu.x(gamma,0,beta,x, TRUE)-att*lambda1.x(gamma,x, TRUE)))^2) +
#       
#       dnorm(x)/p^2*(att*pi.x(alpha,x, TRUE)*lambda1.x(gamma,x, TRUE)*(1-lambda1.x(gamma,x, TRUE))*(att-2*beta.x(alpha,gamma,beta,x)))
#     
#   }
#   
#   out <- (var.y1 + var.y0)/p^2 + integrate(int.fn2, -200, 200)$value
#   
#   return(rep(out, 2))
#   
# }
